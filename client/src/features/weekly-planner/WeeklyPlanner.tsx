...
export function WeeklyPlanner({
  ...
  const schedules: DaySchedule[] = Array.from({ length: 7 }, (_, dayIndex) => ({
    day: new Date(
      new Date().setDate(new Date().getDate() - new Date().getDay() + dayIndex)
    ),
  ...
  return (
    <WeeklyPlannerProvider {...props}>
      <Tabs defaultValue={format(daysOfWeek[0], "EEE", { locale })}>
        <div
          className="relative "
          style={{ "--w-schedule-row-nr": 19 } as React.CSSProperties}
        >
          {isMobile && <MobileTabNavigation />}

          {isMobile ? (
            <MobileDayColumns schedules={schedules} />
          ) : (
            <section className="relative z-20 flex h-[calc(19*50px)] ml-[60px]">
              {schedules.map((day) => (
                <DayColumn
          ...
          {!isMobile && (
            <PlannerGrid
              ...
              extraDecoration={extraDecoration}
            />
          )}
        </div>
      </Tabs>
