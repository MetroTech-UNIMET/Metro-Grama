      ...
      {timeSlots.map((time, index) => (
        <div key={time} className="h-12 relative">
          ...
          {(shouldRenderTime?.(time, index) ?? true) && (
            <time className="inline-block leading-[1] absolute top-0 left-0 transform -translate-y-1/2">
              {time}
            </time>
          )}

          {index < timeSlots.length - 1 && (
            <div className="absolute bottom-0 left-[3.75rem] right-0 h-px bg-gray-200"></div>
          )}
        </div>
      ))}
    </div>
