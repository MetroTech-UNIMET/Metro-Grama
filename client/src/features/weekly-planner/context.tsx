...
type WeeklyPlannerContextProps = (
  | WeeklyPlannerContexUniform
  | WeeklyPlannerContexCustom
) & {
  locale: Locale;
  type: WeeklyPlannerProps["type"];
...
const WeeklyPlannerContext = createContext<
  WeeklyPlannerContextProps | undefined
>(undefined);
...
export const useWeeklyPlannerContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useWeeklyPlannerContext must be used within a WeeklyPlannerProvider"
    );
  }
  return context;
};

export function WeeklyPlannerProvider({
  children,
  ...props
}: {
  ...
  const start_hour =
    props.type === "uniform-interval" ? props.start_hour : props.timeSlots[0];
  const end_hour =
    props.type === "uniform-interval"
      ? props.end_hour
      : props.timeSlots[props.timeSlots.length - 1];

  const interval = props.type === "uniform-interval" ? props.interval : 0;

  const timeSlots =
    props.type === "uniform-interval"
      ? getTimeSlots(start_hour, end_hour, props.interval)
      : props.timeSlots;
